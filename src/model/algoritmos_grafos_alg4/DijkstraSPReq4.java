...
public class DijkstraSPReq4 {
     ...
     */
    public DijkstraSPReq4(EdgeWeightedGraph G, int s) {
        for (Edge e : G.edges()) {
            if (e.tiempoViaje() < 0)
...
//        assert check(G, s);
    ...
    private void relax(Edge e) {
        ...
        if (distTo[w] > distTo[v] + e.tiempoViaje()) {
            distTo[w] = distTo[v] + e.tiempoViaje();
            edgeTo[w] = e;
    ...
    public Stack<Edge> pathTo(int v) {
        ...
        int temp = v;
        for (Edge e = edgeTo[temp]; e != null; e = edgeTo[temp]) {
            path.push(e);
            temp = e.other(temp);
        }
        return path;
...
//    }
  ...
  //      In in = new In(args[0]);
        ...
        DijkstraSPReq4 sp = new DijkstraSPReq4(G, s);
